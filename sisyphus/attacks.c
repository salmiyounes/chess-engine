...
int attacks_to_king_square(ChessBoard *board, const bb b_king) {
    assert(b_king);
    return (attacks_to_square(board, get_lsb(b_king), board->occ[BOTH]) &
...
bb attacks_to_square(ChessBoard *board, int sq, bb occ) {
    return ((get_pawns_attacks(sq, WHITE) & (board->bb_squares[BLACK_PAWN])) |
            ...
            (get_knight_attacks(sq) & (board->bb_squares[WHITE_KNIGHT] |
                                       board->bb_squares[BLACK_KNIGHT])) |
            (get_bishop_attacks(sq, occ) & (board->bb_squares[WHITE_BISHOP] |
            ...
            (get_king_attacks(sq) &
             (board->bb_squares[WHITE_KING] | board->bb_squares[BLACK_KING])));
}